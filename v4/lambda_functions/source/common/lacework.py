...
def setup_initial_access_token(lacework_url, lacework_api_credentials):
    ...
    secret_response = secret_client.get_secret_value(SecretId=lacework_api_credentials)
    ...
    access_token_response = send_lacework_api_access_token_request(
...
def lw_cloud_account_exists_in_orgs(integration_name, lacework_url, access_token, orgs):
    ...
    for org in org_list:
        data_dict = search_lw_cloud_account_by_name(
            integration_name, lacework_url, org, access_token
        )
        if data_dict:
            return True
...
def delete_lw_cloud_account_by_int_guid(
    intg_guid, lacework_url, access_token, sub_account
):
    logger.info("lacework.delete_lw_cloud_account_by_int_guid")
    delete_response = send_lacework_api_delete_request(
        lacework_url, "api/v2/CloudAccounts/" + intg_guid, access_token, sub_account
    )
    ...
    if delete_response.status_code == 204:
        ...
        logger.warning(
            "API response error deleting Config account {} {}".format(
...
def delete_lw_cloud_account_in_orgs(integration_name, lacework_url, access_token, orgs):
    ...
    for org in org_list:
        ...
        if data_dict:
            return delete_lw_cloud_account_by_int_guid(
                data_dict["intgGuid"], lacework_url, access_token, org
            )
    logger.warning("integration name {} not found for deletion.")
    return False


def update_lw_cloud_account_in_orgs(
    integration_name, lacework_url, sub_account_name, access_token, orgs, role_arn, acct
):
    ...
    for org in org_list:
        ...
        if data_dict:
            delete_lw_cloud_account_by_int_guid(
                data_dict["intgGuid"], lacework_url, access_token, org
            )
            ...
            logger.info(
                "Updated acct {} to {} in Lacework. Moved to {}".format(
                    acct, integration_name, sub_account_name
                )
            )
            return True
        else:
            logger.info(
                "integration name {} not found in org {}".format(integration_name, org)
            )
    logger.warning("integration name {} not found for update.")
    return False


def lw_cloud_account_exists(
    integration_name, lacework_url, access_token, sub_account=""
):
...
def add_lw_cloud_account_for_ct(
    integration_name,
    lacework_url,
    sub_account,
    access_token,
    ...
    """.format(
        integration_name, external_id, role_arn, sqs_queue_url
    )
    logger.info(
        "Generate create account {} payload : {}".format(sub_account, request_payload)
    )

    add_response = send_lacework_api_post_request(
        lacework_url, "api/v2/CloudAccounts", access_token, request_payload, sub_account
    )
...
def add_lw_cloud_account_for_cfg(
    integration_name,
    lacework_url,
    account_name,
    access_token,
    ...
    if add_response.status_code == 201:
        ...
        if retry_count < 3:
            retry_count += 1
            time.sleep(5)
            return add_lw_cloud_account_for_cfg(
                integration_name,
                lacework_url,
                account_name,
                access_token,
...
def delete_lw_cloud_account(integration_name, lacework_url, sub_account, access_token):
    ...
    if data_dict:
        return delete_lw_cloud_account_by_int_guid(
            data_dict["intgGuid"], lacework_url, access_token, sub_account
        )
    else:
        logger.warning(
            "Cloud account {} not deleted. int_guid not found.".format(integration_name)
        )
        return False


def search_lw_cloud_account_by_name(
    integration_name, lacework_url, sub_account, access_token
):
    ...
    search_response = send_lacework_api_post_request(
        lacework_url,
        "api/v2/CloudAccounts/search",
        access_token,
        search_request_payload,
        sub_account,
    )
    ...
    if search_response.status_code == 200:
        ...
        if len(data_dict) == 0:
            logger.warning(
                "Cloud account with integration name {} was not found.".format(
                    integration_name
                )
            )
            return False
            ...
            logger.warning(
                "More than one cloud account with integration name {} was found.".format(
                    integration_name
                )
            )
            return False
...
def send_lacework_api_access_token_request(lacework_url, access_key_id, secret_key):
    logger.info(
        "lacework.send_lacework_api_access_token_request: {}".format(lacework_url)
    )
    request_payload = """
        {{
            "keyId": "{}",
    ...
    try:
        return requests.post(
            "https://" + lacework_url + "/api/v2/access/tokens",
...
def send_lacework_api_post_request(
    lacework_url, api, access_token, request_payload, account_name
):
    logger.info(
        "lacework.send_lacework_api_post_request: {} {} {} {}".format(
            lacework_url, api, account_name, request_payload
        )
    )
    try:
        if not account_name:
            return requests.post(
                "https://" + lacework_url + "/" + api,
                headers={
...
def send_lacework_api_delete_request(lacework_url, api, access_token, account_name):
    logger.info(
        "lacework.send_lacework_api_delete_request: {} {} {}".format(
            lacework_url, api, account_name
        )
    )
    try:
        if not account_name:
