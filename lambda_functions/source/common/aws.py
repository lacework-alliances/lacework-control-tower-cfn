...
def find_config_bucket(audit_session, region):
    """
    Finds the bucket starting with 'aws-controltower-config-' in the Audit account.
    ...
    try:
        ...
        for bucket in response['Buckets']:
            if bucket['Name'].startswith('aws-controltower-config-'):
                logger.info(f"Found Config Bucket: {bucket['Name']}")
                return bucket['Name']
                
        raise Exception("Could not find aws-controltower-config bucket in Audit Account.")
    except Exception as e:
